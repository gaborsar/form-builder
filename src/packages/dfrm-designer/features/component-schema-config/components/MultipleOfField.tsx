...
interface MultipleOfFieldProps {
  path: string[];
  node: Node<SchemaTreeNumberNodeData, ComponentSchemaTreeNodeData>;
}

export const MultipleOfField: React.FunctionComponent<MultipleOfFieldProps> = React.memo(
  ({ path, node }) => {
    const dispatch = useDispatch();

    const { data } = node;
    const { multipleOf } = data;
        ...
        dispatch({
          ...
          payload: {
            ...
            node: {
              ...
              data: {
                ...data,
                multipleOf: value === "" ? null : Number.parseInt(value, 0),
              },
            },
          },
        });
    ...
    return (
      <Field>
        ...
        <NumberInput
          name="multipleOf"
          value={multipleOf === null ? "" : `${multipleOf}`}
