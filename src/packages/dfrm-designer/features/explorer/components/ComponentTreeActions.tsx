...
export const ComponentTreeActions: React.FunctionComponent = React.memo(() => {
  const dispatch = useDispatch();

  const onExpandAll = React.useCallback(() => {
    dispatch({ type: "component-tree__expand-all" });
  }, [dispatch]);

  const onCollapseAll = React.useCallback(() => {
    dispatch({ type: "component-tree__collapse-all" });
  }, [dispatch]);
