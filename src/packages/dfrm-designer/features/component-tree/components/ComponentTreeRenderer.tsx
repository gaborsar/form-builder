...
import { useComponentTreeState, useIntlState } from "../../../model";
...
export const ComponentTreeRenderer: React.FunctionComponent = React.memo(() => {
  const { locale } = useIntlState();
  const { query = "", root } = useComponentTreeState();

  const filteredRoot = React.useMemo(() => filterRoot(root, locale, query), [root, locale, query]);

  const [source, setSource] = React.useState<ComponentTreeDndSubject | null>(null);
  const [target, setTarget] = React.useState<ComponentTreeDndSubject | null>(null);
